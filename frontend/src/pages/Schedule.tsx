      ...
      <div className="space-y-4">
        ...
        {scheduled.length === 0 ? (
          ...
          scheduled.map((course) => (
            <Card key={course.courseID} className="border-slate-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
              <CardContent className="p-5 space-y-4">
                {/* Header */}
                <div className="flex items-start justify-between gap-3">
                  <Link to={`/courses/${course.courseID}`} className="flex-1 min-w-0">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-5 gap-3">
          {WEEK_DAYS.map(({ key, label }) => {
            const courses = scheduled.filter((c) => c.days.includes(key));
